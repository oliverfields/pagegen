from Utility import report_error, load_config, SITECONF, CONFROOT, CONTENT_DIR, DIRDEFAULTFILE, TARGETDIR, INCLUDEDIR, load_file, write_file, report_warning
...
class Site:
	...
	def __init__(self, site_dir, config_file):
		...
		if isdir(site_dir):
			...
			self.content_dir=join(site_dir, CONTENT_DIR)
		else:
	...
	def load_pages(self, dir_path, parent):
		...
		for f in file_list:
			...
			elif match(DIRDEFAULTFILE+'[.a-z]*$', f) and dir_path != self.content_dir:
				pass
			elif isfile(f_path):
	...
	def generate_menu(self, pages, page, level=1):
		...
		for p in pages:
			...
			if p.children:
				page.menu+='<li%s><a href="%s">%s</a>' % (css_class, p.url_path, p.title)
				page.menu+='<ul>'
				self.generate_menu(p.children, page, level=level+1)
				page.menu+='</ul>'
				page.menu+='</li>'
			else:
				page.menu+='<li%s><a href="%s">%s</a></li>' % (css_class, p.url_path, p.title)
